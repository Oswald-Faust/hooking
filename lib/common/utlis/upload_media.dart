...
abstract class UploadMedia {
  ...
  static Future<dynamic> _showDialogHandler(
...
class _SelectMedia extends StatelessWidget {
  ...
  Future<dynamic> getContentHandler(
    ...
    try {
      ...
      if (xFile != null) {
        if (kIsWeb) {
          // Sur le web, on retourne directement le XFile
          return xFile;
        }
  ...
  void getContentFromSource(BuildContext context, ImageSource source) async {
    ...
    if (result != null) {
      if (isImage) {
        if (kIsWeb) {
          // Sur le web, on retourne directement le résultat
